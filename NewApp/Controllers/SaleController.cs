...
{
    public class SaleController : SBaseController
    {
        ...
        public void GetMovingValue()
        {
            ...
            {
                _Ds = new DataSet();
                string _SAPID = username.ToString();
                List<Parameters> lstparameters = new List<Parameters>();
                lstparameters.Add(new Parameters("@SAPId", _SAPID));
                _Ds = dataAccess.GetDataSet("SP_GetMovingValue", lstparameters);
                if (_Ds.Tables[0].Rows.Count > 0)
                {
                    //string sVa = Convert.ToDecimal(_Ds.Tables[0].Rows[0].Field<decimal>("TotalOutstanding")).ToString();
                    ViewBag.TotalOut = Convert.ToDecimal(_Ds.Tables[0].Rows[0].Field<decimal>("TotalOutstanding")).ToString();
                    ViewBag.OverdueOut = Convert.ToDecimal(_Ds.Tables[0].Rows[0].Field<decimal>("OverdueOutstanding")).ToString();
                }
        ...
        public ActionResult AgingReportExcel(string sCardCode, string sRadio)
        {
            ...
            var jsonResult = Json(_CustomerList, JsonRequestBehavior.AllowGet);
